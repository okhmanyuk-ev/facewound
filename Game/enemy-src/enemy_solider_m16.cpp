...
#define USEWEAPON WEP_M16

enemy_soldier_m16::enemy_soldier_m16(void)
{
	m_x=0;
	m_y=0;
	m_yvel=-2;
...
void enemy_soldier_m16::Draw(int layer)
{
	if (layer!=0) return;
...
void enemy_soldier_m16::HandleAttacking(void)
{
	if (reloadtimer>0)
	{
	...
	else if (aimangle==targetangle && clip>0)
	{
		...
		else
		{
			...
			fighttimer = PCORE->weapons.m_GunAttributes[USEWEAPON].TimeBetweenShots;
...
void enemy_soldier_m16::StartupVariables(void)
{
	aimangle = -D3DX_PI/5;
